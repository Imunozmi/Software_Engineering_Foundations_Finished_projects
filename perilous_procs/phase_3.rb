def selected_map!(arr, test_prc, return_prc)

    arr.map! do |ele| 
        if test_prc.call(ele)
            return_prc.call(ele) 
        else
            ele
        end
...
def chain_map(num, arr)
    ...
    arr.inject(num) { |acc, prc| prc.call(acc) }
...
def proc_suffix(str, hsh)
    ...
    new_str = str.split(" ").map do |wrd|
        ...
        hsh.each do |key, suff|
            wrd = wrd + suff if key.call(og_wrd)
        end
        wrd
    end
...
def proctition_platinum(arr, *procs)
    ...
    (1..procs.length).each do |i|
        hsh[i] = []
        arr.select! { |ele| !(hsh[i] << ele if procs[i-1].call(ele)) }
    end
...
def procipher(str, proc_hash)
    ...
    new_str = str.split(" ").map do |wrd|
        og_wrd = wrd
        proc_hash.each { |k, v| wrd = v.call(wrd) if k.call(og_wrd) }
        wrd
    end
